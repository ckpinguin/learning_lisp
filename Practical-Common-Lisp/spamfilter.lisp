...
; "Database" access happens mostly here
(defun intern-feature (word)
  (or (gethash word *feature-database*)
	  (setf (gethash word *feature-database*)
			(make-instance 'word-feature :word word))))

; Explicit clearing the database is necessary, because defvar above makes
; the database persistent (re-evaluating defvar does not clear the variable like
; as defparameter does)
...
(defun spam-probability (feature)
  ; Count spam relative to the total messages (0-1) 
  (with-slots (spam-count ham-count) feature
	(let ((spam-frequency (/ spam-count (max 1 *total-spams*)))
		  (ham-frequency (/ ham-count (max 1 *total-hams*))))
	  (/ spam-frequency (+ spam-frequency ham-frequency)))))

; Here comes the Bayes stuff! (Suggested by Robinson)
...
(defun add-directory-to-corpus (dir type corpus)
  (dolist (filename (ch.codehome.pathnames:list-directory dir))
	(add-file-to-corpus filename type corpus)))

(defun test-classifier (corpus testing-fraction)
  (clear-database)
  (let* ((shuffled (ch.codehome.practical:shuffle-vector corpus))
		 (size (length corpus))
...
(defun train-from-corpus (corpus &key (start 0) end)
  (loop for idx from start below (or end (length corpus)) do
	   (destructuring-bind (file type) (aref corpus idx)
		 (train (ch.codehome.practical:start-of-file file *max-chars*) type))))

(defun test-from-corpus (corpus &key (start 0) end)
  (loop for idx from start below (or end (length corpus)) collect
	   (destructuring-bind (file type) (aref corpus idx)
		 (multiple-value-bind (classification score)
			 (classify (ch.codehome.practical:start-of-file file *max-chars*))
		   (list
			:file file
			:type type
